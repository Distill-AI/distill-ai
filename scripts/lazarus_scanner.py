...
IOC_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    # Original Lazarus / BeaverTail markers
    ...
    # main.ts / entry-point dropper (fetch + eval via env var)
    (re.compile(r"atob\s*\(\s*process\.env\b"),          "base64-decoded env var execution (dropper)"),
    (re.compile(r"\bAUTH_API_KEY\b"),                    "AUTH_API_KEY dropper env var"),
    (re.compile(r"import\s*\(\s*['\"]node-fetch['\"]"),  "dynamic node-fetch import"),
    (re.compile(r"\beval\s*\(\s*await\b"),               "eval of awaited fetch payload"),
    (re.compile(r"\beval\s*\(\s*\w+(?:Info|Data|Text|Res|Result|Payload|Proxy)\b"), "eval of named fetch result"),
    # Known C2 IP addresses (hard-coded in dropper or exfil calls)
    (re.compile(r"166\.88\.54\.158|198\.105\.127\.210|23\.27\.202\.27"),  "known C2 IP"),
    (re.compile(r"154\.91\.0\.103|136\.0\.9\.8|166\.88\.4\.2"),           "known C2 IP"),
    (re.compile(r"23\.27\.120\.142|202\.155\.8\.173"),                    "known C2 IP"),
    (re.compile(r"166\.88\.134\.82|188\.43\.33\.249"),                    "known C2 IP"),
]
...
PROCESS_CMDLINE_IOCS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"AUTH_API_KEY"),                    "AUTH_API_KEY dropper var in cmdline"),
    (re.compile(r"atob\s*\("),                       "base64 decode in cmdline"),
    (re.compile(r"node-fetch", re.IGNORECASE),       "node-fetch in cmdline"),
    (re.compile(r"global\['_V'\]|A10-010|A10-2340|_t_t"), "known dropper marker in cmdline"),
    (re.compile(r"166\.88\.54\.158|198\.105\.127\.210|23\.27\.202\.27|154\.91\.0\.103"), "known C2 IP in cmdline"),
]

# Hosts to suggest blocking via the hosts file.  Not auto-written — needs root.
C2_HOSTS = [
    # Crypto RPC endpoints used by the wallet-drainer payload
    "api.trongrid.io",
...
# Known C2 / exfil IP addresses — suggest firewall/hosts blocking.
# Source: community reports of the Lazarus 'Contagious Interview' campaign.
C2_IPS = [
    "166.88.54.158",
    "198.105.127.210",
    "23.27.202.27",
...
# Match any *.config.{js,mjs,cjs,ts} (covers next, postcss, tailwind, vite,
# webpack, rollup, svelte, astro, nuxt, remix, babel, vue, jest, playwright,
# cypress, prettier, commitlint, lint-staged, metro, …) plus a few dash-named
# configs that don't follow the *.config.* convention.
...
# Entry-point source files targeted by the main.ts dropper injection.
ENTRY_POINT_PATTERN = re.compile(
    r"^(?:main|index|server|app|bootstrap)\.[mc]?[jt]sx?$"
)

# Font extensions used to disguise malicious JS files in public/.
FAKE_FONT_EXTENSIONS = {".woff", ".woff2", ".ttf", ".otf", ".eot"}
# Signatures that should never appear inside a real font binary.
FONT_JS_PATTERN = re.compile(
    r"eval\s*\(|require\s*\(|import\s*\(|function\s+\w+\s*\(|=>|atob\s*\("
)

def is_config_target(name: str) -> bool:
    return bool(CONFIG_FILE_PATTERN.match(name))

def is_entry_point(name: str) -> bool:
    return bool(ENTRY_POINT_PATTERN.match(name))
...
def iter_targets(root: Path) -> Iterator[Path]:
    for dirpath, dirnames, filenames in os.walk(root, followlinks=False):
        ...
        rel_dir = Path(dirpath).relative_to(root) if Path(dirpath) != root else Path(".")
        in_public = any(part == "public" for part in rel_dir.parts)
        for fn in filenames:
            if is_config_target(fn) or is_entry_point(fn):
                yield Path(dirpath) / fn
            elif in_public and Path(fn).suffix.lower() in FAKE_FONT_EXTENSIONS:
                yield Path(dirpath) / fn


def scan_file(path: Path) -> FileFinding | None:
    suffix = path.suffix.lower()
    is_font = suffix in FAKE_FONT_EXTENSIONS
    ...
    # Fake font check — a real font file should not contain JS constructs.
    if is_font:
        if FONT_JS_PATTERN.search(text):
            indicators.append("fake font: JS constructs found inside font file")
        if size < 100:
            indicators.append(f"fake font: suspiciously small ({size}B)")
...
def harden(report_lines: list[str]) -> None:
    ...
    report_lines.append(
        f"firewall: block these {len(C2_IPS)} known C2 IPs at your firewall or router:"
    )
    for ip in C2_IPS:
