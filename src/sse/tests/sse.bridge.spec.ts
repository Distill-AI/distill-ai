...
const MockRedis = Redis as unknown as MockInstance;
...
describe('SseService — Redis bridge', () => {
  ...
  describe('onModuleInit()', () => {
    ...
    it('attaches the message listener before connecting', async () => {
      ...
      subMock.on = vi.fn().mockImplementation(() => callOrder.push('on'));
      subMock.connect = vi.fn().mockImplementation(async () => callOrder.push('connect'));
  ...
  describe('emit()', () => {
    ...
    it('falls back to local EventEmitter2 when publish rejects', async () => {
      ...
      pubMock.publish = vi.fn().mockRejectedValue(new Error('Redis down'));
  ...
  describe('subscriber message handler', () => {
    it('re-emits parsed Redis messages into the local EventEmitter2', async () => {
      ...
      const messageHandler = (subMock.on as MockInstance).mock.calls.find(
        ([evt]) => evt === 'message',
