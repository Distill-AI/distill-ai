...
import type { JobType } from '../api/interface/jobs';
...
function LatencyRow({
  ...
  label: string;
  stats: { min: number; p50: number; p95: number; p99: number; max: number; avg: number };
}) {
  return (
  ...
  return (
    <div className="max-w-3xl space-y-10">
        ...
        {algoResult && (
          <div className="rounded-lg border bg-white shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-4 border-t bg-gray-50 text-xs text-gray-500">
              Heap: priority-first ordering. Wheel: time-first, 3600 slots × 1 s, overflow bucket
              for delays &gt; 1 h.
            </div>
          </div>
        )}
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mb-6">
          Creates N real jobs (max_retries=0), waits for every job to reach a terminal state, then
          shows queue-wait and processing latency. Run multiple times to compare job types or
          counts.
        </p>
        ...
        <div className="rounded-lg border bg-white p-6 shadow-sm mb-6">
          ...
          {throughputMutation.isPending && (
            <p className="mt-3 text-xs text-gray-400">
              Creating jobs and waiting for workers to process them — this may take up to 30s for
              N=200…
            </p>
          )}
                ...
                <div className="px-5 py-3 border-b bg-gray-50 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500">
                    ...
                    <span className="font-semibold text-gray-700">
                      {result.throughputPerSec} jobs/sec
                    </span>
