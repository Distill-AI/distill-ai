...
export function MatchedLineRow({ line, thresholds }: MatchedLineRowProps) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg px-4 py-2.5 transition-colors hover:bg-gray-50">
      <span className="w-6 text-right text-xs font-mono text-muted">{position}</span>
      <span className="flex-1 truncate text-sm text-slate-900">{rawText}</span>
